...
# svn $Id: d_ecmwf2roms.m 832 2017-01-24 22:07:36Z arango $
#=========================================================================%
...
# was extracted for the Ligurian Sea (100.5W, 10.5N) to (70.5W, 40.5N)
...
#   ecmwf_era_temp_2000.nc              time, t2m, d2m, ssrd, par
#   ...
...
# run the *_config.m script before
addpath(genpath('/home/abarth/src/roms-matlab/'))

# test with
# >> which nc_read

# full path to the download ERA data file (from the file forcing_ecmwf.py)
InpFile = fullfile(getenv('HOME'), 'projects/Python/ROMS/era_interim_2016_short.nc')
exp_name = '2015'

#domain_name = 'choose_a_name'
#t0 = mjd(2016,04,02)
#t1 = mjd(2016,04,10)
StrDay    = t0;      % starting day to process
EndDay    = t1;     % ending   day to process
...
F(16).output = [domain_name '_Qair_era_' exp_name '.nc'];
F(16).scale  = 1.0;

F(17).Vname  = {'PAR', 'par'};
F(17).Tname  = {'srf_time',  'time'};
F(17).input  = 'ecmwf_era_temp_';
F(17).output = [domain_name '_PAR_era_' exp_name '.nc'];
...
#epoch = datenum(1858,11,17,0,0,0);
...
for n = doFields,
  ...
  while (year <= EndYear)
    ...
    %InpFile  = fullfile(Dir, 'era_interim_2016.nc');
    %InpFile  = fullfile(Dir, inputfile);
