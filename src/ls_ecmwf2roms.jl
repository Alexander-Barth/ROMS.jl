...
Dir = fullfile(getenv('HOME'), 'projects/Python/ROMS');
# same file name as in the file forcing_ecmwf.py
inputfile = 'era_interim_2015.nc'

domain_name = 'choose_a_name'
exp_name = '2015'
StrDay    = tr(1)      % starting day to process
EndDay    = tr(2);     % ending   day to process
...
F(16).output = [domain_name '_Qair_era_' exp_name];
F(16).scale  = 1.0;

F(17).Vname  = {'PAR', 'par'};
F(17).Tname  = {'srf_time',  'time'};
F(17).input  = 'ecmwf_era_temp_';
F(17).output = [domain_name '_PAR_era_' exp_name];
